...
class JobController extends Controller
{
    ...
    public function jobsBySearch(Request $request)
    {
        ...
        $search = $this->sanitize($request->query('search', ''));
        ...
        $cityIdsArray = $this->fetchAllIdsArray($is_featured, 'jobs.city_id',$state_ids);
        ...
        $companyIdsArray = $this->fetchAllIdsArray($is_featured, 'jobs.company_id');
        /*         * ************************************************** */
        ...
        $functionalAreaIdsArray = $this->fetchAllIdsArray($is_featured, 'jobs.functional_area_id');
        ...
        $careerLevelIdsArray = $this->fetchAllIdsArray($is_featured, 'jobs.career_level_id');
        ...
        $jobTypeIdsArray = $this->fetchAllIdsArray($is_featured, 'jobs.job_type_id');
        ...
        $genderIdsArray = $this->fetchAllIdsArray($is_featured, 'jobs.gender_id');
        ...
        $degreeLevelIdsArray = $this->fetchAllIdsArray($is_featured, 'jobs.degree_level_id');
        ...
        $jobExperienceIdsArray = $this->fetchAllIdsArray($is_featured, 'jobs.job_experience_id');
    ...
    {   
        $this->removeAsFavourite($job_slug);
        flash(__('Job has been removed from favorites list'))->success();
        return \Redirect::route('job.detail', $job_slug);
    }

    public function removeAsFavourite($job_slug){
        $user_id = Auth::user()->id;
        FavouriteJob::where('job_slug', 'like', $job_slug)->where('user_id', $user_id)->delete();
    }

    public function removeFromAppliedJob(Request $request, $job_id)
    {
        
        $job = Job::where('id', $job_id)->first();
        
        if($this->cancelApplication($job_id)){
            flash(__('You have successfully canceled your application for this job'))->success();
    ...
    public function cancelApplication($job_id){
        $user_id = Auth::user()->id;
        return JobApply::where('job_id', $job_id)->where('user_id', $user_id)->delete();
    }
    ...
    public function applyJob(Request $request, $job_slug)
    {
        ...
        if ((bool)$user->verified === false) {
            flash(__('Your account is not verified yet. Please verify your account using the link that we sent to your email.'))->error();
            return \Redirect::route('job.detail', $job_slug);
            exit;
        }
        ...
        $currency_codes = CountryDetail::select('countries_details.code')->where('countries_details.code', '!=', null)->orderBy('countries_details.code')->pluck('countries_details.code', 'countries_details.code')->toArray();
        return view('job.apply_job_form')
                        ->with('job_slug', $job_slug)
                        ->with('job', $job)
                        ->with('myCvs', $myCvs)
                        ->with('current_salary', $current_salary)
                        ->with('expected_salary', $expected_salary)
    ...
    public function postApplyJob(ApplyJobFormRequest $request, $job_slug)
    {
        ...
        $jobApply->current_salary = $user->current_salary;
        $jobApply->expected_salary = $user->expected_salary;
