...
class DataArrayHelper
{
    ...
    {
        $array = State::select('states.state', 'states.state_id')->where('states.country_id', '=', $country_id)->lang()->active()->orderBy('states.state')->pluck('states.state', 'states.state_id')->toArray();
    ...
    /*     * **************************** */
    public static function langCmsContentArray($lang)
    {
        $array = CmsContent::select('cms_content.page_id')->where('lang', $lang)->pluck('cms_content.page_id')->toArray();
        return $array;
    }

    public static function langCmsPagesArray($lang)
    {
        $array = Cms::select('cms.id','cms.page_slug')->whereNotIn('id', self::langCmsContentArray($lang))->orderBy('cms.page_slug')->pluck('cms.page_slug', 'cms.id')->toArray();
        return $array;
    }
    /*     * **************************** */
}
