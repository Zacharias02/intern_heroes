    ...
    <div class="modal-content">
        <form class="form" id="add_edit_profile_skill" method="PUT" action="{{ route('update.front.profile.skill', [$profileSkill->id,$user->id]) }}">{{ csrf_field() }}
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary round" onClick="submitProfileSkillForm();">{{__('Update Skill')}} <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
            </div>
        </form>
    </div>
    <!-- /.modal-content --> 
