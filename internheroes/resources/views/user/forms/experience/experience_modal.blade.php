    ...
    <div class="modal-content">
        <form class="form" id="add_edit_profile_experience" method="POST" action="{{ route('store.front.profile.experience', [$user->id]) }}">{{ csrf_field() }}
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary round" onClick="submitProfileExperienceForm();">{{__('Add Experience')}} <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
            </div>
        </form>
    </div>
    <!-- /.modal-content --> 
