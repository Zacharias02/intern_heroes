<h5>{{__('Internship Details')}}</h5>
@if(isset($job))
{!! Form::model($job, array('method' => 'put', 'route' => array('update.front.job', $job->id), 'class' => 'form post-job-form')) !!}
{!! Form::hidden('id', $job->id) !!}
@else
{!! Form::open(array('method' => 'post', 'route' => array('store.front.job'), 'class' => 'form post-job-form')) !!}
@endif
<div class="row">  
    <div class="col-md-12">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'title') !!}"> 
            {!! Form::text('title', null, array('class'=>'form-control', 'id'=>'title')) !!}
            <label>{{__('Internship Title')}}</label>
...
<div class="row">  
    <div class="col-md-12">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'description') !!}"> 
            ...
            <label>{{__('Internship Description')}}</label>
...
<!-- <div class="row">
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'salary_from') !!}" id="salary_from_div"> 
            {!! Form::number('salary_from', null, array('class'=>'form-control', 'id'=>'salary_from', 'placeholder'=>'salary_to', 'maxlength'=>'8', 'minlength'=>'4')) !!}
            <label>{{_('Salary from')}}</label>
            {!! APFrmErrHelp::showErrors($errors, 'salary_from') !!} 
        </div>
    </div>
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'salary_to') !!}" id="salary_to_div">
            {!! Form::number('salary_to', null, array('class'=>'form-control', 'id'=>'salary_to', 'placeholder'=>'salary_to', 'maxlength'=>'8', 'minlength'=>'4')) !!}
            <label>{{_('Salary to')}}</label>
...
<div class="row"> 
    ...
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'degree_level_id') !!}" id="degree_level_id_div"> 
            {!! Form::select('degree_level_id', ['' =>__('Select Educational Level')]+$degreeLevels, null, array('class'=>'form-control', 'id'=>'degree_level_id')) !!}
            <label>Required Educational Level</label>
...
<div class="row"> 
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'functional_area_id') !!}" id="functional_area_id_div"> 
            {!! Form::select('functional_area_id', ['' => __('Select Industry')]+$industries, null, array('class'=>'form-control', 'id'=>'functional_area_id')) !!}
            <label>{{_('Industries')}}</label>
    ...
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'functional_area_id') !!}" id="functional_area_id_div"> 
            ...
            <label>{{_('Functional Areas')}}</label>
            {!! APFrmErrHelp::showErrors($errors, 'functional_area_id') !!} 
        </div>
    </div>
    <div class="col-md-6">
...
<div class="row"> 
    ...
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'num_of_positions') !!}" id="num_of_positions_div"> 
            ...
            <label>{{_('Number of Positions')}}</label>
...
<div class="row"> 
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'gender_id') !!}" id="gender_id_div"> 
            ...
            <label>{{_('Preference')}}</label>
            {!! APFrmErrHelp::showErrors($errors, 'gender_id') !!} 
        </div>
    </div>
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'expiry_date') !!}"> 
                {!! Form::text('expiry_date', null, array('class'=>'form-control datepicker1', 'id'=>'expiry_date1', 'placeholder'=>__('Job expiry date'), 'autocomplete'=>'off', 'readonly'=>'on', 'style'=>'padding-left:10px;font-weight: bold;')) !!}
                <label>{{_('Job expiry date')}}</label>
                {!! APFrmErrHelp::showErrors($errors, 'expiry_date') !!} 
<!--             @if(isset($job->id))
                <?php $expriry = \Carbon\Carbon::parse((isset($job) && $job->expiry_date != '') ? $job->expiry_date: '')->format('M d Y'); ?> 
                {!! Form::text('expiry_date', $expriry, array('class'=>'form-control datepicker', 'id'=>'expiry_date', 'placeholder'=>__('Job expiry date'), 'autocomplete'=>'off')) !!}
                <label>{{_('Job expiry date')}}</label>
                {!! APFrmErrHelp::showErrors($errors, 'expiry_date') !!} 
            @else
                {!! Form::text('expiry_date', null, array('class'=>'form-control datepicker1', 'id'=>'expiry_date1', 'placeholder'=>__('Job expiry date'), 'autocomplete'=>'off', 'readonly'=>'on', 'style'=>'border:2px solid green;')) !!}
                <label>{{_('Job expiry date')}}</label>
                {!! APFrmErrHelp::showErrors($errors, 'expiry_date') !!} 
            @endif -->
...
<div class="row"> 
    <div class="col-md-12">
        <div class="formrow">
            <button type="submit" class="btn btn-secondary round">{{__('Post Internship')}} <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        return d.date() != fm.date() && fm.isSame(fmEnd.format('YYYY-MM-D')) ? fm.add(1, 'd') : fm;  
        }

        var nextMonth = moment.addRealMonth(moment());
        document.getElementById('expiry_date1').value = nextMonth.format('YYYY-MM-D');
        ...
        filterLangStates(<?php echo old('state_id', (isset($job)) ? $job->state_id : $siteSetting->default_state_id); ?>);
    });
